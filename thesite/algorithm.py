...
def writeXml(filepath,comNum,dict, username):
    if os.path.exists("/home/ddjian/demo/static/%s/" % username):
        shutil.rmtree("/home/ddjian/demo/static/%s/" % username)
    os.mkdir("/home/ddjian/demo/static/%s/" % username)
...
def writeJS(filepath,comNum,dict,username):
    if os.path.exists("/root/vina/demo/static/%s/" % username):
        shutil.rmtree("/root/vina/demo/static/%s/" % username)
    os.mkdir("/root/vina/demo/static/%s/" % username)
...
def algorithm(k, filename, alpha=None,labelfile=None, username=None):
    ...
    writeJS("/root/vina/demo/static/%s/piechart.js"%username,comNum,dict,username)#
    top = findtop(L, result)
    least=findleast(L)
    # print top
    ...
    writexls(L,L.__len__(),labelfile,listresult,result,"/root/vina/demo/static/dv/%s/nodes.xls"%username,"/root/vina/demo/static/dv/%s/communites.xls"%username)
