    ...
    if request.method == "POST":
        ...
        if uf.is_valid():
            ...
            if User.objects.exclude(pk=request.user.pk).filter(username=username).exists():
                ...
                user.save()
                os.mkdir("/root/vina/demo/static/dv/%s"%username)
                os.mkdir("/root/vina/demo/data/%s"%username)
...
def details(request):       # details页面
    if request.user.is_authenticated():
            user = request.user
    filename = "/root/vina/demo/static/dv/%s/labeldata.txt" % user
    tag = 0
    if os.path.exists("/root/vina/demo/static/dv/%s/ok" % user):
        tag = 1
    if tag == 0:
...
def datasamplea(request):
    # username = request.user.username
    filename = '/root/vina/demo/static/dataset/a.rar'     # Select your file here.
    wrapper = FileWrapper(file(filename))
    response = HttpResponse(wrapper, content_type='text/plain')
    response['Content-Length'] = os.path.getsize(filename)
    return response
...
def datasampleb(request):
    # username = request.user.username
    filename = '/root/vina/demo/static/dataset/b.rar'     # Select your file here.
    wrapper = FileWrapper(file(filename))
    response = HttpResponse(wrapper, content_type='text/plain')
    response['Content-Length'] = os.path.getsize(filename)
    return response
...
def datasamplec(request):
    # username = request.user.username
    filename = '/root/vina/demo/static/dataset/c.rar'     # Select your file here.
    wrapper = FileWrapper(file(filename))
    response = HttpResponse(wrapper, content_type='text/plain')
    response['Content-Length'] = os.path.getsize(filename)
    return response
...
def datasampled(request):
    # username = request.user.username
    filename = '/root/vina/demo/static/dataset/d.rar'     # Select your file here.
    wrapper = FileWrapper(file(filename))
    response = HttpResponse(wrapper, content_type='text/plain')
    response['Content-Length'] = os.path.getsize(filename)
    return response
...
def datasamplee(request):
    # username = request.user.username
    filename = '/root/vina/demo/static/dataset/e.rar'     # Select your file here.
    wrapper = FileWrapper(file(filename))
    response = HttpResponse(wrapper, content_type='text/plain')
    response['Content-Length'] = os.path.getsize(filename)
    return response
