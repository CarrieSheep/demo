...
def drawChart(result,total,listresult,username=None):
    ...
    plt.savefig("/home/ddjian/demo/static/dv/%s/chart.png"%username)     # save as png
    plt.clf()
...
def drawea(L,result,listresult,labelresult):
    ...
    for i in range(0,result.__len__(),1):
        ...
        gg.vs["label"]=label
        bbox=(10,10,590,590)
        plot(gg,"/home/ddjian/demo/static/dv/community%s.png"%(i+1),bbox=bbox,layout =layout,vertex_size=Vsize,edge_color="grey",vertex_color = color_dict[c%50])
        del matrix
    print density
    return density
...
def drawn(g, listresult, vex,labelfile,top,username=None):
    ...
    #     layout=g.layout("fr")
        ...
        print username
        p.save("/home/ddjian/demo/static/dv/%s/SNA.png"%username)
        p.remove(g)
...
def Nodecom(comLin,result,username=None):
    ...
    p.save("/home/ddjian/demo/static/dv/%s/com.png"%username)
    p.remove(g)
    return g
...
def denCom(density,comLin,result,username=None):
    ...
    p.add(g,
         ...
         byrow = True,
         vertex_color = [color_dict[i % 50] for i in range(0,result.__len__(),1)])
    p.save("/home/ddjian/demo/static/dv/%s/dencom.png"%username)
    p.remove(g)
    return g
...
def drawCentralization(L,result,username=None):
    ...
    p.add(g,
         ...
         byrow = True,
         vertex_color = [color_dict[i % 50] for i in range(0,result.__len__(),1)])
    p.save("/home/ddjian/demo/static/dv/%s/center.png"%username)
    p.remove(g)
...
def drawe(L,result,listresult,labelresult,i,username=None):
        ...
        for j in labelresult[i]:
           label.append(j)
        gg.vs["label"]=label
        file_o= codecs.open('/root/vina/demo/static/js/drawgraph/graph.js', 'r','utf-8')
        text =file_o.read()
        print text
        tag = 1
        if os.path.exists("/root/vina/demo/static/js/drawgraph/%s" % username):
             tag = 0
        if tag == 1:
             os.mkdir("/root/vina/demo/static/js/drawgraph/%s" % username)
        file_object= codecs.open('/root/vina/demo/static/js/drawgraph/%s/graph.js'%username, 'w','utf-8')
        ...
        file_object.write("}\n")
        file_object.close()
        if( os.path.exists("/root/vina/demo/static/dv/%s/community%s.png"%(username,(i+1)))):
            del matrix
            return result[i].__len__() , gg.ecount(),density, dc
        plot(gg,"/root/vina/demo/static/dv/%s/community%s.png"%(username,(i+1)),layout =layout,vertex_size=Vsize,edge_color="grey",vertex_color = color_dict[c%50])
        del matrix
        return result[i].__len__() , gg.ecount(),density, dc
